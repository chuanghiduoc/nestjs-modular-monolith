...
import { Relabel__MODULE_PASCAL__UseCase } from '../application/relabel-__MODULE_KEBAB__.use-case';
...
import { Relabel__MODULE_PASCAL__RequestDto } from './dto/relabel-__MODULE_KEBAB__.request.dto';
...
export class __MODULE_PASCAL__Controller {
  constructor(
    ...
    private readonly relabel__MODULE_PASCAL__: Relabel__MODULE_PASCAL__UseCase,
  ) {}
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Relabel a __MODULE_KEBAB__ record the caller owns.' })
  @ApiOkResponse({ type: __MODULE_PASCAL__ResponseDto })
  async relabel(
    @CurrentUser() caller: AuthenticatedSession | undefined,
    @Param('id', new ParseUUIDPipe({ version: '7' })) id: string,
    @Body() body: Relabel__MODULE_PASCAL__RequestDto,
  ): Promise<__MODULE_PASCAL__ResponseDto> {
    const session = requireCaller(caller);

    const view = await this.relabel__MODULE_PASCAL__.execute({
